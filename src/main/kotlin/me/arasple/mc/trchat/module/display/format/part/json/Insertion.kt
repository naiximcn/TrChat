...
class Insertion(override val content: String, override val condition: Condition?) : Part() {
    ...
    override fun process(tellraw: TellrawJson, player: Player, vararg vars: String, message: String): String? {
        if (dynamic) {
            tellraw.insertion(content.replacePlaceholder(player).replace("\$message", message).replaceWithOrder(*vars))
        } else {
            tellraw.insertion(content.replace("\$message", message).replaceWithOrder(*vars))
        }
        return null
    }
}