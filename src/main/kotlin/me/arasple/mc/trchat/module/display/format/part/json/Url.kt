...
class Url(override val content: String, override val condition: Condition?) : Part() {
    ...
    override fun process(builder: TextComponent.Builder, sender: CommandSender, vararg vars: String, message: String) {
        if (!dynamic || sender !is Player) {
            builder.clickEvent(ClickEvent.openUrl(content.replace("\$message", message).replaceWithOrder(*vars)))
        } else {
            builder.clickEvent(ClickEvent.openUrl(content.replacePlaceholder(sender).replace("\$message", message).replaceWithOrder(*vars)))
        }
    }
}